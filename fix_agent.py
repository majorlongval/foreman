...
class FixAgent:
    ...
    def fix_pr(self, pr) -> bool:
        ...
        cycles = self._count_fix_cycles(pr)
        if cycles >= MAX_FIX_CYCLES:
            log.info(f"  ⚠️ Max fix cycles ({MAX_FIX_CYCLES}) reached — skipping")
            self.stats["skipped"] += 1
            return True
        ...
        try:
            for filepath in affected_files:
                ...
                for attempt in range(2):
                    ...
                    log.info(f"  💰 Cost: ${response.cost:.4f} | {model}")
                    ...
                    patched_content, errors, applied_count = apply_patches(current_content, patches)
                    total_patches = len(patches)
                    if errors:
                        log.warning(f"  ⚠️ Attempt {attempt+1}: {applied_count}/{total_patches} patches applied. Errors: {errors}")
                        prompt += f"\n\nPatch application failed ({applied_count}/{total_patches} successful):\n" + "\n".join(errors) + "\nFix your search strings."
                        continue
                    ...
                    log.info(f"  ✅ {applied_count}/{total_patches} patches applied successfully")
