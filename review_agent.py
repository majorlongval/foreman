...
logging.getLogger("completion").setLevel(logging.WARNING)
    ...
    def review_pr(self, pr) -> bool:
        ...
        try:
            if self._already_reviewed_head(pr):
                log.info(f"  ⏭️ Skipping: Head commit {pr.head.sha[:7]} already reviewed")
                self.stats["skipped"] += 1
                return True

            fix_cycles = self._count_fix_cycles(pr)
            if fix_cycles >= MAX_FIX_CYCLES:
                log.info(f"  ⏭️ Skipping: Max fix cycles ({MAX_FIX_CYCLES}) exhausted")
                if not self.dry_run:
            ...
            log.info(f"  💰 Cost: ${usage1.total_cost:.4f} (accumulated: ${self.cost.total_cost:.4f})")
            ...
            if review_data_1["critical_count"] > 0 or review_data_1["important_count"] > 0:
                log.info(f"  Verdict: {review_data_1['verdict']} (🔴 {review_data_1['critical_count']}, 🟡 {review_data_1['important_count']}, 💡 {review_data_1['suggestion_count']})")
                if not self.dry_run:
                    log.info(f"  Action: Posting review comment to PR #{pr.number}")
            ...
            log.info(f"  💰 Cost: ${usage2.total_cost:.4f} (accumulated: ${self.cost.total_cost:.4f})")
            ...
            if review_data_2["critical_count"] > 0 or review_data_2["important_count"] > 0:
                log.info(f"  Verdict: {review_data_2['verdict']} (🔴 {review_data_2['critical_count']}, 🟡 {review_data_2['important_count']}, 💡 {review_data_2['suggestion_count']})")
                if not self.dry_run:
                    log.info(f"  Action: Posting review comment to PR #{pr.number}")
            ...
            # Approval
            log.info(f"  Verdict: APPROVE (🔴 0, 🟡 0, 💡 {review_data_2['suggestion_count']})")
            ...
            log.info(f"  Action: Posting {approve_event} and adding '{LABEL_REVIEWED}' label to PR #{pr.number}")
    ...
    def run_once(self) -> dict:
        ...
        for pr in queue:
            try:
                ...
                log.error(f"  ❌ Unhandled exception in review_pr for PR #{pr.number}: {e}", exc_info=True)
                self.stats["failed"] += 1
        ...
        try:
            while True:
                try:
                    ...
                    log.error(f"❌ Error in review loop iteration: {e}", exc_info=True)
